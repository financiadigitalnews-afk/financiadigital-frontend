  ...
  return (
    ...
    >
      ...
      <span className="flex items-center gap-2.5">
        ...
        <span className="text-sm">Live Podcast</span>
      </span>

      <span className="rounded-full bg-white/20 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-white ring-1 ring-white/30">
        Live
      </span>
    </Link>
  );
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl supports-[backdrop-filter]:bg-white/88">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-2 sm:px-5 lg:px-6 xl:px-8">
          <Link to="/" className="flex min-w-0 items-center">
            <div className="w-[150px] sm:w-[190px] lg:w-[220px] xl:w-[240px]">
              <img
                src={logo1}
          ...
          <div className="hidden flex-1 items-center justify-end gap-4 lg:flex">
            ...
            <div className="flex items-center gap-3 pl-1 text-sm font-bold text-slate-700">
              {utilityNavItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.slug}
                  className="rounded-full px-2.5 py-1.5 transition hover:bg-slate-100 hover:text-slate-950"
                >
                  {item.name}
                </Link>
              ))}
          ...
          <button
            ...
            className="inline-flex shrink-0 rounded-full border border-slate-200 p-2 text-slate-900 transition hover:bg-slate-50 lg:hidden"
        ...
        >
          <nav className="mx-auto flex w-full max-w-7xl items-center justify-center gap-1.5 px-6 py-2 xl:px-8">
            {desktopNavItems.map((item) => {
              ...
              return hasDropdown ? (
                <button
                  ...
                  className={`inline-flex shrink-0 items-center gap-1 rounded-full px-3.5 py-2 text-sm font-black transition ${
                    isActive
                ...
                <Link
                  ...
                  to={item.slug}
                  onMouseEnter={() => setActiveDropdown(null)}
                  className="inline-flex shrink-0 rounded-full px-3.5 py-2 text-sm font-black text-slate-800 transition hover:bg-slate-100 hover:text-slate-950"
                >
                  {item.name}
                </Link>
              );
